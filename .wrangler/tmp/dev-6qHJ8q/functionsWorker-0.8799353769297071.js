...
// .wrangler/tmp/bundle-WwCDvb/checked-fetch.js
...
// .wrangler/tmp/pages-jR3Fc9/functionsWorker-0.8799353769297071.mjs
...
async function getEncryptionKey(env) {
  ...
  if (/^[0-9a-fA-F]{64}$/.test(raw)) {
    ...
    for (let i2 = 0; i2 < 32; i2++) keyBytes[i2] = parseInt(raw.slice(i2 * 2, i2 * 2 + 2), 16);
  } else {
...
var init_path = __esm({
  "api/[[path]].ts"() {
    ...
    __name2(randomCode, "randomCode");
    __name2(sendEmail, "sendEmail");
    __name2(escapeHtml2, "escapeHtml");
    __name2(appEmailTemplate, "appEmailTemplate");
    __name2(verificationCodeBlock, "verificationCodeBlock");
    __name2(getEncryptionKey, "getEncryptionKey");
    __name2(encrypt, "encrypt");
    __name2(decrypt, "decrypt");
    __name2(bumpTaskOrdersForTab, "bumpTaskOrdersForTab");
    ...
    __name2(todayUtc, "todayUtc");
    __name2(dayFromQuery, "dayFromQuery");
    ApiValidationError = class extends Error {
      static {
        __name(this, "ApiValidationError");
      }
      static {
        __name2(this, "ApiValidationError");
      }
      status;
    ...
    __name2(optionalString, "optionalString");
    __name2(validateId, "validateId");
    __name2(ensureStrongPassword, "ensureStrongPassword");
    __name2(requireAuth, "requireAuth");
    __name2(getTabAccess, "getTabAccess");
    __name2(requireTabAccess, "requireTabAccess");
    __name2(getFirstEditableTabAccess, "getFirstEditableTabAccess");
    ...
    onRequest = /* @__PURE__ */ __name2(async (context) => {
      ...
      const addCors = /* @__PURE__ */ __name2((r) => {
      ...
      try {
        ...
        if (path === "/tabs/reorder" && request.method === "PUT") {
          ...
          for (let i2 = 0; i2 < filtered.length; i2++) {
            await env.DB.prepare('UPDATE tabs SET "order" = ? WHERE id = ? AND user_id = ?').bind(i2, filtered[i2], userId).run();
          }
          return addCors(jsonResponse({ ok: true }));
        }
        ...
        if (path === "/tasks/reorder" && request.method === "PUT") {
          ...
          for (let i2 = 0; i2 < taskIds.length; i2++) {
            await env.DB.prepare('UPDATE tasks SET "order" = ? WHERE id = ? AND tab_id = ?').bind(i2, taskIds[i2], tabId).run();
          }
          return addCors(jsonResponse({ ok: true }));
        }
  ...
  return function(pathname) {
    ...
    var _loop_1 = /* @__PURE__ */ __name2(function(i22) {
      ...
      if (key.modifier === "*" || key.modifier === "+") {
        params[key.name] = m[i22].split(key.prefix + key.suffix).map(function(value) {
          return decode(value, key);
        });
      } else {
        params[key.name] = decode(m[i22], key);
      }
    }, "_loop_1");
...
// .wrangler/tmp/bundle-WwCDvb/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-WwCDvb/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.8799353769297071.js.map
